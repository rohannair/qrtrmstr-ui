...
test('Defaults', t => {
  ...
  t.truthy(Array.isArray(state.users), 'Users property is an array');
  t.deepEqual(app(state, action), state, 'No mutation if action type is unrecognized');


});

test(USERS_RETRIEVED, t => {
...
test(NEW_USER_CREATED, t => {
  ...
  t.plan(2);
  t.truthy(app(state, action).users.total === state_after.users.total, 'Add new user to total');
  t.deepEqual(app(state, action).users.results, state_after.users.results, 'Insert new user');
});

test(PASSWORD_RESET, t => {
